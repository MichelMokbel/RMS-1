                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if (! $orderLocked)
                                                @if ($item->status === 'Pending')
                                                    <flux:button size="2xs" wire:click="setItemStatus({{ $item->id }}, 'InProduction')">{{ __('Start') }}</flux:button>
                                                    <flux:button size="2xs" variant="ghost" wire:click="setItemStatus({{ $item->id }}, 'Cancelled')">{{ __('Cancel') }}</flux:button>
                                                @elseif ($item->status === 'InProduction')
                                                    <flux:button size="2xs" wire:click="setItemStatus({{ $item->id }}, 'Ready')">{{ __('Ready') }}</flux:button>
                                                    <flux:button size="2xs" variant="ghost" wire:click="setItemStatus({{ $item->id }}, 'Cancelled')">{{ __('Cancel') }}</flux:button>
                                                @elseif ($item->status === 'Ready')
                                                    <flux:button size="2xs" wire:click="setItemStatus({{ $item->id }}, 'Completed')">{{ __('Complete') }}</flux:button>
                                                    <flux:button size="2xs" variant="ghost" wire:click="setItemStatus({{ $item->id }}, 'Cancelled')">{{ __('Cancel') }}</flux:button>
                                                @endif
                                            @endif
                                        </div>
