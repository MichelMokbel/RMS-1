...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function resyncAll(SubscriptionPaymentLinkService $service): void
    {
        $this->resyncing = true;
        MealSubscription::query()->each(fn ($sub) => $service->resyncMealsUsed($sub));
...
<div class="app-page space-y-6">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            <flux:button wire:click="resyncAll" wire:loading.attr="disabled" size="sm" variant="ghost">
                <span wire:loading.remove wire:target="resyncAll">{{ __('Resync All') }}</span>
                <span wire:loading wire:target="resyncAll">{{ __('Resyncing…') }}</span>
            </flux:button>
    ...
    <div class="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900 space-y-3">
        <div class="app-filter-grid">
            ...
            <div>
                ...
                <x-number-input wire:model.live="branch_id" type="number" class="w-32" />
            </div>
            <div>
        ...
        <table class="w-full min-w-full table-auto divide-y divide-neutral-200 dark:divide-neutral-800">
            <thead class="bg-neutral-50 dark:bg-neutral-800/90">
                <tr>
                    ...
                    <th class="px-3 py-2 text-center text-xs font-semibold uppercase tracking-wider text-neutral-700 dark:text-neutral-100">{{ __('Meals Used') }}</th>
