...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function with(): array
    {
        ...
        $expiredNotRenewedSubscriptions = 0;
        ...
        $subscriptionAttentionItems = collect();
        ...
        if ($canViewPrograms) {
            ...
            $expiredNotRenewedSubscriptions = (clone $subscriptionsQuery)->expiredNotRenewed()->count();
            ...
            $expiredNotRenewedQuery = MealSubscription::query()
                ->with(['customer:id,name'])
                ->withRenewalState()
                ->expiredNotRenewed()
                ->orderByDesc('end_date')
                ->orderByDesc('created_at');
            $applyBranchScope($expiredNotRenewedQuery);

            $lowMealSubscriptions = $lowMealQuery->get()->map(function (MealSubscription $subscription) {
                $subscription->attention_reason = 'low_meals';

                return $subscription;
            });

            $expiredNotRenewedItems = $expiredNotRenewedQuery->get()->map(function (MealSubscription $subscription) {
                $subscription->attention_reason = 'expired_not_renewed';

                return $subscription;
            });

            $subscriptionAttentionItems = $lowMealSubscriptions
                ->concat($expiredNotRenewedItems)
                ->values();
        ...
        $subscriptionsCard = $this->card(
            ...
            [
                ...
                __('Expired not renewed: :count', ['count' => number_format($expiredNotRenewedSubscriptions)]),
            ],
            __('Branch-aware'),
            true
        );
        ...
        return [
            ...
            'subscriptionAttentionItems' => $subscriptionAttentionItems,
        ];
    }
...
<div class="app-page space-y-8 py-8">
    ...
    @if ($subscriptionAttentionItems->isNotEmpty())
        <section class="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm dark:border-amber-800/60 dark:bg-amber-950/30">
            <div class="flex items-center gap-2 mb-4">
                ...
                <h2 class="text-sm font-semibold text-amber-800 dark:text-amber-300">{{ __('Subscription Attention Needed') }}</h2>
            </div>
            <div class="divide-y divide-amber-200 dark:divide-amber-800/50">
                @foreach ($subscriptionAttentionItems as $sub)
                    @php
                        $remaining = $sub->plan_meals_total !== null
                            ? (int) $sub->plan_meals_total - (int) ($sub->meals_used ?? 0)
                            : null;
                    @endphp
                    <div class="flex items-center justify-between gap-4 py-2.5 first:pt-0 last:pb-0">
                        ...
                        <div class="flex items-center gap-3 text-sm text-amber-700 dark:text-amber-300">
                            @if ($sub->attention_reason === 'low_meals')
                                ...
                                <span class="rounded-full bg-amber-200 px-2.5 py-0.5 text-xs font-semibold text-amber-800 dark:bg-amber-800/50 dark:text-amber-200">
                                    {{ __('Low meals') }}
                                </span>
                            @else
                                <span>{{ $sub->subscription_code }}</span>
                                <span class="rounded-full bg-rose-100 px-2.5 py-0.5 text-xs font-semibold text-rose-700 dark:bg-rose-900/40 dark:text-rose-300">
                                    {{ __('Expired not renewed') }}
                                </span>
                            @endif
