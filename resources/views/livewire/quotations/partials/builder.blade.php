    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(380px,0.85fr)]">
        <div class="space-y-5" x-show="tab === 'edit' || window.innerWidth >= 1024">
            ...
            <section class="rounded-lg border border-primary-200 bg-primary-50/40 p-5 shadow-sm dark:border-primary-800 dark:bg-primary-950/20">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div x-data="{ amount: '{{ number_format(((int)data_get($items, '0.unit_price_cents', 0))/100, 2, '.', '') }}' }"><flux:input x-model="amount" x-on:change="$wire.set('items.0.unit_price_cents', Math.round((parseFloat(amount)||0)*100))" type="number" min="0" step="0.01" :label="__('Price per person (QAR)')" /></div>
                    <flux:input wire:model.live.debounce.400ms="items.0.quantity" type="number" min="1" step="1" :label="__('Minimum order (persons)')" />
                </div>
            ...
            <section class="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="space-y-3">
                    @forelse($items as $index => $item)
                        <div wire:key="quote-item-{{ $index }}" class="grid gap-3 rounded-lg bg-neutral-50 p-3 dark:bg-neutral-800 sm:grid-cols-12">
                            ...
                            <div class="sm:col-span-2"><flux:input wire:model.live.debounce.500ms="items.{{ $index }}.quantity" type="number" min="0.001" step="0.001" :label="__('Quantity')" /></div>
                            <div class="sm:col-span-2"><flux:input wire:model.live.debounce.500ms="items.{{ $index }}.unit" :label="__('Unit')" /></div>
                            <div class="sm:col-span-2" x-data="{ amount: '{{ number_format(((int)($item['unit_price_cents'] ?? 0))/100, 2, '.', '') }}' }"><flux:input x-model="amount" x-on:change="$wire.set('items.{{ $index }}.unit_price_cents', Math.round((parseFloat(amount)||0)*100))" type="number" min="0" step="0.01" :label="__('Price QAR')" /></div>
                            <div class="sm:col-span-2" x-data="{ amount: '{{ number_format(((int)($item['discount_cents'] ?? 0))/100, 2, '.', '') }}' }"><flux:input x-model="amount" x-on:change="$wire.set('items.{{ $index }}.discount_cents', Math.round((parseFloat(amount)||0)*100))" type="number" min="0" step="0.01" :label="__('Discount QAR')" /></div>
                ...
                <div class="mt-4 grid gap-3 border-t border-neutral-200 pt-4 sm:grid-cols-3 dark:border-neutral-700"><div><label class="mb-1 block text-sm font-medium">{{ __('Document discount') }}</label><select wire:model.live="quotation_discount_type" class="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm dark:border-neutral-600 dark:bg-neutral-800"><option value="">{{ __('None') }}</option><option value="fixed">{{ __('Fixed amount') }}</option><option value="percentage">{{ __('Percentage') }}</option></select></div>@if($quotation_discount_type)<div x-data="{ value: '{{ number_format($quotation_discount_value / 100, 2, '.', '') }}' }"><flux:input x-model="value" x-on:change="$wire.set('quotation_discount_value', Math.round((parseFloat(value)||0)*100))" type="number" min="0" step="0.01" :label="$quotation_discount_type === 'fixed' ? __('Discount QAR') : __('Discount %')" /></div>@endif<div class="self-end text-right"><p class="text-sm text-neutral-500">{{ __('Quotation total') }}</p><p class="text-xl font-semibold">{{ number_format(((int)($totals['total_cents'] ?? 0))/100, 2) }} QAR</p></div></div>
            </section>
            @endif
                ...
                <div data-quotation-block-list class="grid grid-cols-12 gap-3">
                    @foreach($blocks as $blockIndex => $block)
                        ...
                        <div data-block-id="{{ $block['id'] }}" wire:key="block-{{ $block['id'] }}" class="col-span-12 {{ $spanClass }} {{ $newRowClass }} min-w-0 rounded-lg border border-neutral-200 p-3 dark:border-neutral-700">
                            ...
                            @elseif($block['type'] === 'spacer') <flux:input wire:model.live="blocks.{{ $blockIndex }}.settings.height_mm" type="number" min="2" max="100" :label="__('Height (mm)')" />
