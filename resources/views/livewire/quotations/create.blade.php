...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function mount(BranchAccessService $access): void
    {
        abort_unless(auth()->user()?->can('quotations.manage'), 403);
        $this->branch_id = $access->allowedBranchIds(auth()->user())[0] ?? 0;
        abort_if($this->branch_id === 0, 403, __('No permitted branch is available.'));
        ...
        else $this->addFreeTextItem();
    }

    public function updatedBranchId(BranchAccessService $access): void
    {
        abort_unless($access->canAccessBranch(auth()->user(), $this->branch_id), 403);
        $this->template_version_id = null;
        $this->applyDefaultTemplate();
    }

    public function updatedTemplateVersionId(mixed $versionId): void
    {
        if (blank($versionId)) {
            $this->applyDefaultTemplate();
            return;
        }
        $companyId = Branch::query()->whereKey($this->branch_id)->value('company_id');
        $version = DocumentTemplateVersion::query()
            ->whereHas('template', fn ($query) => $query->where('company_id', $companyId)->where('type', 'quotation')->where('is_active', true))
    ...
    public function addCatalogItem(int $id): void
    {
        ...
        $this->items[] = [
            ...
            'unit' => $menuItem->unit ?: 'each',
            'quantity' => '1.000',
            'unit_price_cents' => (int) round(((float) $menuItem->selling_price_per_unit) * 100),
            'discount_cents' => 0,
            'catalog_snapshot' => ['code' => $menuItem->code, 'name' => $menuItem->name, 'arabic_name' => $menuItem->arabic_name],
        ];
        $this->catalog_search = '';
    }
    ...
    public function addBlock(string $type): void
    {
        ...
        $this->blocks[] = $this->newBlock($type);
    }
    ...
    {
        if ($this->isMenuProposal()) $this->ensureMenuPricingItem();
        $quotation = $drafts->create(auth()->user(), $this->payload());
        session()->flash('status', __('Draft quotation created. Changes now save automatically.'));
        $this->redirect(route('quotations.edit', $quotation), navigate: true);
    }
    ...
    public function finalize(QuotationDraftService $drafts, QuotationLifecycleService $lifecycle): void
    {
        if ($this->isMenuProposal()) $this->ensureMenuPricingItem();
        $quotation = $drafts->create(auth()->user(), $this->payload());
        $lifecycle->finalize(auth()->user(), $quotation);
    ...
    public function previewHtml(): string
    {
        ...
        $items = collect($calculation['items'])->map(fn (array $item) => [
            ...$item,
            'unit_price_minor' => (int) ($item['unit_price_cents'] ?? 0),
            'line_discount_minor' => (int) ($item['discount_cents'] ?? 0),
            'line_total_minor' => (int) ($item['line_total_cents'] ?? 0),
        ])->all();

        $profile = CompanyDocumentProfile::query()->where('company_id', $companyId)->first();
        ...
        return app(QuotationHtmlRenderer::class)->render([
            'schema_version' => 1,
            'quotation' => ['number' => null, 'issue_date' => $this->issue_date, 'valid_until' => $this->valid_until, 'currency' => 'QAR'],
            ...
            'assets' => app(QuotationAssetService::class)->snapshotMap(DocumentAsset::query()->where('company_id', $companyId)->whereIn('id', $assetIds)->get()),
        ]);
    }
    ...
    public function with(BranchAccessService $access): array
    {
        $companyId = Branch::query()->whereKey($this->branch_id)->value('company_id');
        return [
            'branches' => Branch::query()->whereIn('id', $access->allowedBranchIds(auth()->user()))->where('is_active', true)->orderBy('name')->get(),
            'templates' => DocumentTemplate::query()->with('currentVersion')->where('company_id', $companyId)->where('type', 'quotation')->where('is_active', true)->orderByDesc('is_default')->orderBy('name')->get(),
            ...
            'documentAssets' => DocumentAsset::query()->where('company_id', $companyId)->where('kind', 'image')->latest()->limit(100)->get(),
        ];
    }

    private function applyDefaultTemplate(): void
    {
        $companyId = Branch::query()->whereKey($this->branch_id)->value('company_id');
        $template = DocumentTemplate::query()->with('currentVersion')->where('company_id', $companyId)->where('type', 'quotation')->where('is_active', true)->orderByDesc('is_default')->first();
        if ($template?->currentVersion) $this->applyVersion($template->currentVersion);
        else {
            $this->blocks = collect(['company_header', 'quotation_metadata', 'recipient_details', 'items_table', 'totals', 'terms', 'signature_lines'])->map(fn ($type) => $this->newBlock($type))->all();
            $this->page_settings = config('quotations.paper');
            $this->styles = ['font_family' => 'DejaVu Sans', 'font_size' => 10, 'default_direction' => 'auto'];
            $this->table_columns = ['description' => true, 'unit' => true, 'quantity' => true, 'unit_price' => true, 'discount' => true, 'total' => true];
        }
    }

    private function applyVersion(DocumentTemplateVersion $version): void
    {
        $this->template_version_id = $version->id;
        $this->page_settings = $version->page_settings ?: [];
        $this->styles = $version->styles ?: [];
        $this->table_columns = $version->table_columns ?: [];
        $this->blocks = $this->withBlockLayoutDefaults($version->blocks ?: []);
        if ($this->isMenuProposal()) $this->ensureMenuPricingItem();
    }

    private function newBlock(string $type): array
    {
        $content = in_array($type, ['rich_text', 'terms'], true) ? ['type' => 'doc', 'content' => [['type' => 'paragraph']]] : null;
        return ['id' => (string) Str::uuid(), 'type' => $type, 'direction' => 'auto', 'settings' => $this->layoutDefaults($type), 'content' => $content];
    }
...
@include('livewire.quotations.partials.builder', ['pageTitle' => __('New quotation'), 'isExisting' => false])
