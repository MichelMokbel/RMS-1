...
new class extends Component {
    ...
    {
        $this->tab = in_array($tab, array_keys($this->tabs()), true) ? $tab : 'chart';
        $this->resetErrorBag();
            ...
            <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_220px]">
                <div class="flex flex-wrap gap-2">
                    @foreach ($tabs as $tabKey => $label)
                        <button type="button" wire:click="setTab('{{ $tabKey }}')" class="{{ $tab === $tabKey ? 'bg-neutral-900 text-white dark:bg-neutral-100 dark:text-neutral-900' : 'bg-white text-neutral-700 dark:bg-neutral-900 dark:text-neutral-200' }} rounded-md border border-neutral-200 px-3 py-2 text-sm font-medium shadow-sm dark:border-neutral-700">
                            {{ $label }}
                        </button>
                    @endforeach
                </div>
