...
<body>
    ...
    @include('reports.print-header', ['reportTitle' => 'Customer Statement'])
    <div class="meta">Generated: {{ $generatedAt->format('Y-m-d H:i') }} | Customer: {{ $customer?->name ?? '—' }} | Filters: {{ json_encode($filters) }}</div>
    <table>
        <thead>
            <tr>
                <th>Date</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="right">—</td>
                <td class="right">—</td>
                <td class="right">{{ $formatCents($balance) }}</td>
            </tr>
            @forelse ($statement['entries'] as $entry)
                ...
                <tr>
                    ...
                    <td class="right">{{ $formatCents($entry['debit_cents']) }}</td>
                    <td class="right">{{ $formatCents($entry['credit_cents']) }}</td>
                    <td class="right">{{ $formatCents($balance) }}</td>
                </tr>
            @empty
