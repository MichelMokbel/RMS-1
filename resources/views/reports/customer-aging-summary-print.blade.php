...
<body>
    ...
    @include('reports.print-header', ['reportTitle' => 'Customer Aging Summary'])
    ...
    <table>
        ...
        <tfoot>
            <tr>
                ...
                <td class="right"><strong>{{ $formatCents((int) $rows->sum('current')) }}</strong></td>
                <td class="right"><strong>{{ $formatCents((int) $rows->sum('bucket_1_30')) }}</strong></td>
                <td class="right"><strong>{{ $formatCents((int) $rows->sum('bucket_31_60')) }}</strong></td>
                <td class="right"><strong>{{ $formatCents((int) $rows->sum('bucket_61_90')) }}</strong></td>
                <td class="right"><strong>{{ $formatCents((int) $rows->sum('bucket_90_plus')) }}</strong></td>
                <td class="right"><strong>{{ $formatCents((int) $rows->sum('total')) }}</strong></td>
            </tr>
        </tfoot>
    </table>
