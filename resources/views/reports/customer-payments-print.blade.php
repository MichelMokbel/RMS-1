...
<body>
    <div class="no-print">
        ...
        <a class="btn" href="{{ route('reports.customer-payments') }}">Back to Report</a>
    </div>
    @include('reports.print-header', ['reportTitle' => 'Customer Payments Report'])
        ...
        <tbody>
            @forelse ($payments as $payment)
                ...
                <tr>
                    ...
                    <td class="right">{{ $formatCents($payment->amount_cents) }}</td>
                    <td class="right">{{ $formatCents($allocated) }}</td>
                    <td class="right">{{ $formatCents($remaining) }}</td>
                </tr>
            @empty
                <tr><td colspan="8">No customer payments found.</td></tr>
            @endforelse
        </tbody>
        @if ($payments->count() > 0)
            <tfoot>
                <tr>
                    ...
                    <td class="right">{{ $formatCents($payments->sum(fn ($payment) => (int) ($payment->allocated_sum ?? 0))) }}</td>
                    <td class="right">{{ $formatCents($payments->sum(fn ($payment) => (int) $payment->amount_cents - (int) ($payment->allocated_sum ?? 0))) }}</td>
                </tr>
            </tfoot>
        @endif
    </table>
