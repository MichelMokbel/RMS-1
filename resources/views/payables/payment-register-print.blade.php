...
<head>
    ...
    <title>{{ __('Payments') }} — {{ __('Filtered Print') }}</title>
    <style>
...
<header class="meta">
    <div>
        <h1>{{ __('Payments') }}</h1>
        <p>{{ __('Filtered table print') }} · {{ trans_choice(':count record|:count records', $payments->count(), ['count' => $payments->count()]) }}</p>
    </div>
    ...
    <tbody>
        @forelse($payments as $payment)
            <tr>
                ...
                <td class="right">{{ number_format((float) $payment->amount, 2) }} {{ strtoupper($payment->currency_code ?: $payment->company?->base_currency ?: config('pos.currency', 'QAR')) }}</td>
