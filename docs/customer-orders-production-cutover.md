...
The final 2026 09 26 production snapshot is `ilwlvmmy_inventory (4).sql` with SHA 256 `c167ceb3ed860d6e8b8ef4a3b5421e14cc9080e0a7d9c1e7af914bfa7f83220c`. Its original copy had 190 tables, 2,358 columns, 1,085 index entries, 237 foreign keys, 13 triggers, and 122 migration rows. The same 67 row manifest was reverified against this exact snapshot. After baselining and applying the remaining migrations, the upgraded copy had 221 tables, 3,000 columns, 1,405 index entries, 385 foreign keys, 13 triggers, and 218 migration rows. Every pre existing business table retained its row count except one permission cache row invalidated by expected permission seeding. Expected reference seeding added 16 permissions and 20 role permission links. A second migration run reported nothing to migrate. The exact upgraded artifact was restored to `layla_rms_prod` on the GCP database VM. Its 221 table row count fingerprint matched the rehearsal copy exactly. The recoverable pre import snapshot is `/mnt/docker-data/backups/layla-rms/layla_rms_prod-pre-final-import-20260926T130300Z.sql.gz`, and the upgraded restore artifact is `/mnt/docker-data/backups/layla-rms/layla_rms_prod-upgraded-20260926.sql.gz`.

That snapshot also confirmed three manually applied order sheet changes that were absent from the migration ledger: excluded order IDs, quantity portion type, and extra portion type. Their verified migration names are included in the baseline manifest. The structured delivery location, production display permission, and order label migrations were not baselined because their effects were not present in the source snapshot. They ran normally as forward migrations.

The final rehearsal found 26 active menu items without a branch assignment. This remains a catalog cleanup gate for normal menu ordering, not a blocker for deploying RMS with that feature disabled. Core foreign key checks and both posted ledger balancing checks passed. Customer identity review found three customers without normalized phone values, 101 historical verification timestamps that are not trusted SMS proof, and legacy user linked order references without a customer link. These are review inputs and must not be converted into invented identity proof or allowed to block ordinary order creation. Delivery notes are classified as current customer owned records and move through the existing audited customer merge flow.
