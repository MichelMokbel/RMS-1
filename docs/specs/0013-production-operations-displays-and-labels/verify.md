...
On 2026-09-13 the Windows setup extension passed 39 focused Laravel tests with 259 assertions on a disposable MySQL 8.4 database. This included automatic print device creation, generated setup packaging, service credential rotation, one branch access, print only token enforcement, denial from non print POS APIs, existing label snapshots, queue retries, claim replay, acknowledgements, idempotency, and the full POS readiness suite. PowerShell execution and physical output remain part of the Windows workstation check below because this development host is not Windows.
