...
Give kitchen and pastry staff dedicated read only screens that show only what they need to prepare. Add compact order labels that open the normal print dialog on whichever device is using RMS, with printer and label dimensions configured in RMS. Prices remain visible to authorized managers but never appear on worker screens or labels. The existing local agent remains an optional unattended printing path rather than a requirement for manual printing.
...
* [Order labels](0013-order-labels.md) defines direct browser printing, label content, printer setup, and the optional unattended queue path.
...
* **AC-7**: An administrator can create a label format for an owned company and branch, select its department, resolution, media mode, exact media dimensions, and default copy count, then preview a sample label. Changes are revision protected and audited. Agent activation and terminal state do not gate direct browser printing.
* **AC-8**: An authorized operator can open the device's normal browser print dialog for one ordinary or pastry order, or for the eligible orders on one service date. The output uses the selected label dimensions and contains the Layla Kitchen name, order number, service date and time, customer name, destination, compact item quantities, copy number, and a small machine readable order reference. It never contains a price, discount, payment method, invoice balance, or customer phone number.
...
**Chosen option**: Dedicated worker displays with direct browser label printing and an optional unattended queue.

Keep the current management pages for authorized operators. Route kitchen and pastry workers to smaller read only screens. For manual labels, RMS serves a fixed size HTML print document and the operator's browser opens the normal local print dialog. Retain the terminal based queue only as an optional future unattended path.
...
| `/order-labels/print/{sourceType}/{sourceId}` | GET | source type, source ID, label format, copies | fixed size HTML that opens the local browser print dialog | `order-labels.print` plus branch access | 403, 404, 422 |
| `/order-labels/print/batch` | GET | source type, branch, date, label format, copies, search | one fixed size browser print page per eligible order and copy | `order-labels.print` plus branch access | 403, 404, 422 |
...
| Render page size | browser print page width and height | selected branch label format media dimensions. Continuous media height is calculated from content within the configured minimum and maximum |
| Select physical printer | available local printer | the operating system print dialog on the device running the browser |
...
* The server never opens a connection to a restaurant printer. For manual printing, the authenticated browser receives a print document and the local operating system owns printer selection. The optional agent initiates its own outbound connection when unattended printing is enabled.
...
* Label happy path: an authorized operator opens one order through a selected label format, the browser launches the local print dialog, and the output is correctly sized and price free without creating a queue job, verifies **AC-7**, **AC-8**.
...
8. [x] Add direct one order and date batch browser printing using selected label dimensions, local print dialog invocation, branch authorization, price free projections, and no queue mutation or activation gate, satisfies **AC-1**, **AC-7**, **AC-8**.
9. [ ] Run the complete automated, browser, printer, role, branch, responsive, optional queue recovery, and deployment verification matrix, satisfies **AC-1** through **AC-13**.
...
* Silent unattended printing still requires the optional Windows background task to remain online inside the restaurant.
...
* The first release uses manual single order and service date batch printing through the normal browser print dialog. It does not automatically print on payment or order creation.
