...
The journal presents one row per accounting event rather than one row per ledger line. Each row shows a readable transaction type, business reference, supplier and branch context, the debit account side, the credit account side, and the event amount. Multi-account debit or credit sides list their individual amounts within the account cell. Reversals remain separate audit events and are labeled `VOID` so an original, its reversal, and a corrected repost cannot be mistaken for accidental duplicates. Internal subledger entry IDs and raw source keys are omitted from the user-facing report.
...
Journal and category report amounts are displayed with three decimal places. The underlying journal data retains the ledger's four-decimal precision. Branchless AP cheque clearance entries are included in company reports for administrators and scheduled delivery, but excluded from branch scoped non administrator reports.
