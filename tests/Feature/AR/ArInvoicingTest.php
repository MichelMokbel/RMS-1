...
it('applies a bulk discount fix to draft and issued invoices and records an audit trail', function () {
    ...
    $first = $invoices->createDraft(
        branchId: 1,
        customerId: $customer->id,
        items: [
            ['description' => 'Service A', 'qty' => '1.000', 'unit_price_cents' => 10000, 'discount_cents' => 500, 'tax_cents' => 0, 'line_total_cents' => 9500],
        ],
        actorId: $user->id,
    );

    $second = $invoices->createDraft(
        branchId: 1,
        customerId: $customer->id,
        items: [
            ['description' => 'Service B', 'qty' => '1.000', 'unit_price_cents' => 8000, 'discount_cents' => 0, 'tax_cents' => 0, 'line_total_cents' => 8000],
        ],
        actorId: $user->id,
    );
    ...
    expect($first->invoice_discount_type)->toBe('percent');
    expect($first->invoice_discount_value)->toBe(1000);
    expect($first->invoice_discount_cents)->toBe(950);
    expect($first->discount_total_cents)->toBe(1450);
    expect($first->total_cents)->toBe(8550);
    expect($first->balance_cents)->toBe(8550);

    expect($second->invoice_discount_type)->toBe('percent');
    expect($second->invoice_discount_value)->toBe(1000);
