...
it('enforces valid order transitions and logs ops events', function () {
    ...
    $svc->advanceOrder($order, 'InProduction', $user->id);
    ...
    $svc->advanceOrder($order, 'Ready', $user->id);
    $order->refresh();
    expect($order->status)->toBe('Ready');

    // Delivery can go OutForDelivery -> Delivered
    $svc->advanceOrder($order, 'OutForDelivery', $user->id);
    $order->refresh();
    expect($order->status)->toBe('OutForDelivery');

    $svc->advanceOrder($order, 'Delivered', $user->id);
    $order->refresh();
...
it('enforces valid item transitions and logs ops events', function () {
    ...
    $svc = app(OrderWorkflowService::class);
    $svc->setItemStatus($item, 'InProduction', $user->id);
    $item->refresh();
    expect($item->status)->toBe('InProduction');

    $svc->setItemStatus($item, 'Ready', $user->id);
    $item->refresh();
    expect($item->status)->toBe('Ready');

    $svc->setItemStatus($item, 'Completed', $user->id);
    $item->refresh();
