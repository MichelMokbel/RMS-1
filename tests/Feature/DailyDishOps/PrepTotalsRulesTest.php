...
it('prep totals include only Confirmed and InProduction orders', function () {
    ...
    $row = $totals->firstWhere('menu_item_id', 100);
...
it('prep totals can exclude subscription or manual orders', function () {
    ...
    expect($onlyManual->firstWhere('menu_item_id', 200))->toBeNull();
    expect($onlyManual->firstWhere('menu_item_id', 201))->not->toBeNull();
    ...
    expect($onlySub->firstWhere('menu_item_id', 201))->toBeNull();
    expect($onlySub->firstWhere('menu_item_id', 200))->not->toBeNull();
});
