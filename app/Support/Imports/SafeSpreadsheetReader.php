...
class SafeSpreadsheetReader
{
    ...
    private function sheetRows(string $content, array $shared, array $dateStyles, bool $date1904, int &$totalCells): array
    {
        ...
        foreach ($sheetData->row as $xmlRow) {
            ...
            foreach ($this->spreadsheetChildren($xmlRow)->c as $cell) {
                ...
                if (isset($cellBody->f)) {
                    throw new RuntimeException('Formulas are not allowed in import workbooks.');
                }
    ...
    private function assertSafeEntries(ZipArchive $zip): void
    {
        ...
        for ($index = 0; $index < $zip->numFiles; $index++) {
            ...
            if ($normalizedName === 'xl/vbaproject.bin'
                || str_starts_with($normalizedName, 'xl/macrosheets/')
                || str_starts_with($normalizedName, 'xl/activex/')
                || str_starts_with($normalizedName, 'xl/ctrlprops/')) {
                throw new RuntimeException('Macros and active content are not allowed in import workbooks.');
            }
