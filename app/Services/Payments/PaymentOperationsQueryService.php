    ...
    public function query(User $actor, array $filters = []): Builder
    {
        ...
        if ($view !== 'all') {
            ...
            $query->whereNotNull('operations_tracking')
                ->whereRaw("JSON_UNQUOTE(JSON_EXTRACT(operations_tracking, '$.issues.processing.attention_at')) <= ?", [$now])
                ->whereRaw("COALESCE(JSON_UNQUOTE(JSON_EXTRACT(operations_tracking, '$.issues.processing.resolved_at')), 'null') = 'null'");
        }
